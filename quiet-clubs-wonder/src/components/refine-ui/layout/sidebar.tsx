...
function SidebarItemCollapsible({ item, selectedKey }: MenuItemProps) {
  ...
  const [isOpen, setIsOpen] = useState(() => {
    ...
    const savedState = localStorage.getItem(`sidebar-collapsed-${name}`);
    return savedState !== 'false'; // Default to open
  });

  // Save state to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem(`sidebar-collapsed-${name}`, isOpen.toString());
  }, [name, isOpen]);
